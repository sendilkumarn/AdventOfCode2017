...
impl Tree {    
    pub fn find_root_node(&self) -> Option<&Node> {
    ...
    pub fn get_sub_node_weight(&self, node: &Node) -> i32 {
        let mut visiting_list = Vec::new();
        visiting_list.push(node);

        let mut weight = 0i32;

        while let Some(node) = visiting_list.pop() {
            weight += node.weight;
            for child in &node.children {
                visiting_list.push(self.nodes.get(child.as_str()).unwrap());
            }
        }
    ...
    pub fn identify_faulty(&self, node: &Node) -> Option<(&Node, i32)> {
        let mut weights: HashMap<i32, Vec<&Node>> = HashMap::new();

        for child in &node.children {
            let child = self.nodes.get(child.as_str()).unwrap();

            weights
                .entry(self.get_sub_node_weight(child))
        ...
        if let Some(second) = w.next() {
            let node = *first.1.iter().next().unwrap();
...
ykudi (46)";    
    let tree_node_map = parse(input);
    let tree = Tree { nodes: tree_node_map };
    let mut root_node = tree.find_root_node();
    let mut diff = 0i32;
    println!("{:?}", root_node);

    while let Some((new_node, new_diff)) = tree.identify_faulty(root_node.unwrap()) {
        root_node = Some(new_node);
        diff = new_diff;
    };

    println!("{}", root_node.unwrap().weight + diff);

}

fn parse(input: &str) -> HashMap<String, Node> {
    let mut tree_nodes: HashMap<String, Node> = HashMap::new();
    let lines: Vec<&str> = input.split("\n").collect();
    for line in &lines {
        let mut parent_child= line.split(" -> ");
        let parent_weight: Vec<&str> = parent_child.next().unwrap().split(" ").collect();
        let parent: &str = parent_weight.get(0).unwrap().trim();
        let weight: i32 = parent_weight.get(1).unwrap().trim().replace("(", "").replace(")","").parse::<i32>().unwrap();
        let children: Vec<&str> = parent_child.next().iter().flat_map(|t| t.split(", ")).collect();
        
        for t in &children {
            let n = tree_nodes.entry(t.to_string()).or_insert_with(Node::default);
            n.parent = Some(parent.to_string());
        }
        
        let n = tree_nodes.entry(parent.to_string()).or_insert_with(Node::default);
        n.name = parent.to_string();
        n.weight = weight;
        n.children.extend(
            children.into_iter().map(|s| s.to_string()),
        );
    }
    tree_nodes
}