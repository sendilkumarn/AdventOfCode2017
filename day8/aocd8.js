const out = `ioe dec 890 if qk > -10
gif inc -533 if qt <= 7
itw dec 894 if t != 0
...
lines.forEach(line => {
	...
	let reg = instruction[0].trim();
	insMap[instruction[4]] = instruction[4] in insMap ? insMap[instruction[4]] : 0;
	let regVal =  eval(`${insMap[instruction[4]]} ${instruction[5]} ${instruction[6]}`) ? parseInt(instruction[2].trim()) : 0;
	let operation = instruction[1] === 'inc' ? '+' : '-';
	let initialValue = (reg in insMap) ? insMap[reg] : 0;
	regVal = eval(`${initialValue} ${operation} ${regVal}`);
	insMap[reg] = regVal;
	if(regVal > m) {
		m = regVal;
	}
